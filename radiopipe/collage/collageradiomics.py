...
class Collage:
    ...
    def __init__(
        ...
        if img_array.shape[0] < min_x_y_size or img_array.shape[1] < min_x_y_size:
            raise Exception(
                f"Image size ({img_array.shape[0]}x{img_array.shape[1]}) unsupported. Image must be a minimum of a 50x50 for collage to run."
            )
        ...
        min_3D_slices = 3
        if (
            self._img_array.shape[0] < self._haralick_window_size
        ...
        if numberOfValues > 2:
            logger.info(
                f"Warning: Mask is not binary. Considering all {numberOfValues} nonzero values in the mask as a value of True."
            )
        ...
        cropped_mask_array = thresholded_mask_array[
            self.mask_min_y : self.mask_max_y,
    ...
    def _calculate_haralick_textures(self, dominant_angles):
        ...
        dominant_angles_binned = (
            (dominant_angles - dominant_angles_min)
            / (dominant_angles_max - dominant_angles_min + np.finfo(float).eps)
            * (num_unique_angles - 1)
        )
    ...
    def execute(self):
        ...
        logger.debug(
            f"Calculating dominant gradient angles using SVD for each image patch of size {svd_radius}x{svd_radius}"
        )
        ...
        haralick_features = np.empty(
            angles_shape[0:3]
            + (
                13,
                2 if self.is_3D else 1,
            )
        )
        ...
        collage_output[
            mask_min_y:mask_max_y, mask_min_x:mask_max_x, mask_min_z:mask_max_z, :, :
        ] = haralick_features
